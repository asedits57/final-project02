...
type PracticeAction =
    ...
    | { type: "SET_FILL_INPUT"; payload: string }
...
const initialState: (totalTime: number) => PracticeState = (totalTime) => ({
    questions: [],
    loading: true,
    currentIdx: 0,
    answers: {},
...
function practiceReducer(state: PracticeState, action: PracticeAction): PracticeState {
    switch (action.type) {
        ...
        case "SET_CURRENT_IDX":
            return { ...state, currentIdx: action.payload, fillInput: "" };
        case "SUBMIT_ANSWER":
            if (state.showResult[action.qId]) return state;
            return {
                ...state,
                answers: { ...state.answers, [action.qId]: action.answer },
                showResult: { ...state.showResult, [action.qId]: true }
            };
        case "SET_FILL_INPUT":
            return { ...state, fillInput: action.payload };
        case "SET_IS_PLAYING":
            return { ...state, isPlaying: action.payload };
        case "TICK":
            if (state.timeLeft <= 0 || state.isFinished) return state;
            if (state.timeLeft <= 1) return { ...state, timeLeft: 0, isFinished: true };
...
const PracticeTest = () => {
    ...
    useEffect(() => {
        const loadQuestions = async () => {
            try {
                ...
                dispatch({ type: "SET_ERROR", payload: "Failed to load practice questions. Please check your connection." });
            }
        };
        loadQuestions();
    }, [levelLabel]);
    ...
    const submitAnswer = useCallback(
        (ans: string) => {
            if (!q) return;
            dispatch({ type: "SUBMIT_ANSWER", qId: q.id, answer: ans });
        },
        [q]
    );
    ...
    const playAudio = useCallback(() => {
        ...
        u.rate = 0.9;
        u.onend = () => dispatch({ type: "SET_IS_PLAYING", payload: false });
    ...
    const goNext = () => {
        if (currentIdx < questions.length - 1) {
            dispatch({ type: "SET_CURRENT_IDX", payload: currentIdx + 1 });
        }
    };
    const goPrev = () => {
        if (currentIdx > 0) {
                        ...
                        <div className="space-y-4">
                            <div className="flex flex-col sm:flex-row gap-4">
                                <input
                                    ...
                                    onChange={(e) => dispatch({ type: "SET_FILL_INPUT", payload: e.target.value })}
                                    disabled={answered}
                            ...
                            {q.blanks && !answered && (
                                <div className="flex flex-wrap gap-2 items-center mt-3">
                                    ...
                                    {q.blanks.map((b) => (
                                        <button
                                            key={b}
                                            onClick={() => dispatch({ type: "SET_FILL_INPUT", payload: b })}
