...
type SpeakingAction =
    ...
    | { type: "TICK_RECORDING" }
    | { type: "STOP_RECORDING" }
    | { type: "SET_FEEDBACK"; payload: Feedback | null }
    | { type: "SET_TRANSCRIPT"; payload: string }
    | { type: "SET_EVALUATING"; payload: boolean }
    | { type: "NEXT_PROMPT"; totalPrompts: number }
    | { type: "RESET" }
...
function speakingReducer(state: SpeakingState, action: SpeakingAction): SpeakingState {
    switch (action.type) {
        case "START_PREP":
            return { ...state, phase: "prep", prepLeft: PREP_TIME };
        case "TICK_PREP":
            return { ...state, prepLeft: Math.max(0, state.prepLeft - 1) };
        case "START_RECORDING":
            return { ...state, phase: "recording", elapsed: 0, transcript: "" };
        case "TICK_RECORDING":
        ...
        case "NEXT_PROMPT":
            if (state.currentIndex < action.totalPrompts - 1) {
                return { 
                    ...state, 
...
const SpeakingModule = () => {
    ...
    const { 
        currentIndex, phase, prepLeft, elapsed, 
        isFinished, feedback, transcript, isEvaluating 
    } = state;
    ...
    useEffect(() => {
        if (speakingPrompts.length > 0) {
            ...
            if (saved) dispatch({ type: "SET_INDEX", payload: Math.min(parseInt(saved), speakingPrompts.length - 1) });
        }
    }, [speakingPrompts]);
    ...
    const stopRecording = async () => {
        ...
        try {
            ...
            dispatch({ type: "SET_FEEDBACK", payload: fallbackFeedback });
    ...
    const handleNext = () => {
        clearTimer();
        dispatch({ type: "NEXT_PROMPT", totalPrompts });
